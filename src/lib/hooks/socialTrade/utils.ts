...
import { ETHERSCAN_API_KEY, POLYGON_ETHERSCAN_API_KEY, ProjectId_2 } from "../../../utils/constants";
...
const proxy_contract_abi = [{"inputs":[{"internalType":"address","name":"admin","type":"address"}],"stateMutability":"nonpayable","type":"constructor"},{"anonymous":false,"inputs":[{"indexed":true,"internalType":"address","name":"implementation","type":"address"}],"name":"Upgraded","type":"event"},{"stateMutability":"payable","type":"fallback"},{"inputs":[],"name":"admin","outputs":[{"internalType":"address","name":"","type":"address"}],"stateMutability":"nonpayable","type":"function"},{"inputs":[],"name":"implementation","outputs":[{"internalType":"address","name":"","type":"address"}],"stateMutability":"nonpayable","type":"function"},{"inputs":[{"internalType":"address","name":"_logic","type":"address"},{"internalType":"bytes","name":"_data","type":"bytes"}],"name":"initialize","outputs":[],"stateMutability":"payable","type":"function"},{"inputs":[{"internalType":"address","name":"newImplementation","type":"address"}],"name":"upgradeTo","outputs":[],"stateMutability":"nonpayable","type":"function"},{"inputs":[{"internalType":"address","name":"newImplementation","type":"address"},{"internalType":"bytes","name":"data","type":"bytes"}],"name":"upgradeToAndCall","outputs":[],"stateMutability":"payable","type":"function"}];
const aaveDepositAbi = ["function deposit(address asset, uint256 amount, address onBehalfOf, uint16 referralCode)"];
...
export const getOldInputs = async (network: any, txHash: any, signer: any) => {
    try {
        if (network === 'polygon') {
            ...
            const txIndex = await BigNumber.from(reciept.transactionIndex).toHexString();
            const zeroX = '0x';
            const txBlockNumber = await zeroX.concat(reciept.blockNumber.toString(16));
...
export const isProxyContract = async (contractAddress: any, ABI: any, signer: any) => {
    try {
        let provider: any = await getCurrentProvider();
        if (!provider) throw('provider not fetched');
        const iface = new ethers.utils.Interface(ABI);
        const contract = new ethers.Contract(contractAddress, proxy_contract_abi, signer);

        console.log('contract', contract);
        const implementation_contract_address = await contract.connect(signer).implementation.call()
...
export const checkAllowance = async (erc20Address: any, from: any, spender: any, amount: any) => {
    try {
        let provider: any = await getCurrentProvider();
        if (!provider) throw('provider not fetched');
...
export const approve = async (erc20Address: any, signer: any, spender: any, amount: any) => {
    try {
